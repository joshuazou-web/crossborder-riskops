...
**让有限的风控人力优先核查真正重要的案件，让 AI 只做有依据、可审计、不能越权的助手。**
...
| | |
| --- | --- |
| **业务场景** | 跨境支付风险运营与反洗钱预警调查 |
| **要解决的问题** | 预警太多、人力有限；既要提高风险发现能力，也要控制误报、积压和错误拦截 |
| **产品方案** | 支付状态机与精确账务计算 + 确定性风险规则 + 可解释模型 + 案件分流与人工复核 + 受限 AI 助手 + 审计追踪 |
| **我的工作** | 独立完成业务场景拆解、数据与规则设计、产品流程、AI 权限边界、评测体系、中英文工作台及项目文档 |
| **技术实现** | Python、DuckDB、pandas、scikit-learn、Streamlit、Plotly；核心检测与案件排序不依赖语言模型 |
| **验证范围** | 支付风险模块覆盖 6,000 笔合成交易与 5 个随机种子；反洗钱模块覆盖 4 万笔合成转账 × 3 个种子；451 个自动化测试 |
| **明确边界** | 未接入真实支付网络，未在生产环境运行，不构成 AML、KYC、PCI 或其他监管合规证明 |

### 我具体做了什么

- **把支付业务建成可执行模型：** 用 8 个生命周期状态、11 条合法状态转换、幂等控制、汇率锁定、
  退款、拒付和结算对账，描述一笔跨境支付从授权到结算可能发生的异常。
- **把风险判断写成可解释规则：** 设计 9 类支付风险信号、20 条确定性规则和 6 类反洗钱可疑
  交易模式；每条信号都保留触发字段、判断理由、风险等级和反证。
- **把“模型分数”变成运营流程：** 将结果分为自动放行、补充材料、人工复核和先行止付，并为
  案件队列加入去重、归并、优先级、处理容量、服务时限和申诉机制。
- **把 AI 权限限制落实到代码：** AI 可以整理案件、解释信号、引用证据和选择弃权，但数据结构、
  权限校验和审计模块共同阻止它放行、止付、退款或结案。
- **把项目效果变成可复现证据：** 建立跨随机种子评测、混淆矩阵、逐规则消融、误报分析、
  对抗性模型输出测试和 README 指标一致性检查。
- **完成端到端产品交付：** 提供中英文运营工作台、案件调查页、策略调参、审计记录、命令行工具、
  演示视频、架构说明、数据字典和真实性边界文档。

### 这个项目体现的金融科技产品能力

1. **理解业务，而不只是在页面上接一个模型。** 支付生命周期、金额精度、汇率、重复扣款、
   退款与拒付都进入底层数据和规则设计。
2. **把风险效果与运营成本放在同一张表里。** 不只看召回率，也同时报告精确率、人工复核率、
   自动放行漏检率、案件积压和误拦申诉结果。
3. **设计可以真正落地的人机分工。** AI 提升的是阅读证据和调查案件的效率，最终处置权仍由
   获授权人员承担。
4. **能从需求走到验证闭环。** 从业务建模、产品流程和交互界面，到规则、数据、评测、测试、
   文档和演示，形成一套可运行、可复核的完整原型。
...
![调查工作台二十秒：案件摘要、把它排到队列第一位的八个因子，以及证据与反证的并排布局
](docs/screenshots/demo-zh.gif)

*这二十秒是调查工作台：一个案件、把它排到队列第一位的八个因子——全部摊开，
所以不认同排序的人能看到是哪一项造成的——然后是证据，以及**并排而不是在它下面**的反证。
这个位置就是设计本身：被队列压着的复核人自上而下读，而且会提前停。*

**[▶ 完整演示 · 2 分 02 秒](docs/demo/demo-zh-narrated.mp4)** · 中文界面、中文配音与字幕 ·
...
只命中一种可疑交易模式的案件，在「相互印证」上得分为**零**。
同一账户上同时出现多种相互独立的可疑交易模式，是系统能够提供的最强风险依据，因此应当显著提高调查优先级。

### 为什么召回率不是 1.000，以及这反而更可信

生成器的第一版把每个场景都造在检测阈值的正中间，**六类可疑交易模式全部达到 100% 召回率**。
...
| --- | --- | --- | --- |
| `clear` 明显高于阈值 | 阈值之内 | **1.000 ± 0.000** | 应该高 |
| `borderline` 临近阈值 | 刚好压线 | **0.952 ± 0.027** | 阈值真正买到的东西 |
| `below_threshold` 明确低于阈值 | 故意造在外面 | **0.162 ± 0.054** | **应该低** |
...
1. **认真地建模一笔跨境支付。** 八个生命周期状态、十一条合法状态转移、幂等模型、带锁定窗口的
   ...
   不同货币的小数位数也作为基础数据维护，因为假设所有货币都保留两位小数，是跨境支付中常见的系统错误。
2. **用人人看得懂的规则识别风险。** 9 个信号族、20 条确定性规则，每条都写明自己是从哪几个字段算出来的。
   **检测环节没有任何语言模型参与。**
3. **引入可解释模型，同时量化它究竟带来了多少增量。** 15 个具名特征的逻辑回归，每个分数都能拆出各特征贡献。
4. **用纯函数完成案件分流。** `auto_release` / `manual_review` / `request_information` / `auto_hold`
...
6. **回答分析师的第二个问题。** 简报回答第一个;追问对话拿到的是**更宽的上下文包**——
   ...
   **系统没有的数据就明确说明无法回答，不用相邻信息敷衍**；无论分析师如何措辞，**都不接受对方交出决策权**。
7. **人始终在环里，并且带理由码。** 四种角色、按风险等级的 SLA、申诉流程，以及**被测量而非假设**的误拦回收率。
8. **所有关键动作都写入哈希链式审计日志**，包括 AI 助手与人工处置意见不一致的情况。
9. **让指标能够重新计算。** 本文件中的每个数字都由 `python -m riskops eval` 生成，
   而且有一个测试会在本文件与评测输出不一致时失败。
10. **提供完整的中英文界面。** 每页右上角均可一键切换。**翻译的不只是标签，还有判断依据与说明**，
...
| 角色为 `ai_copilot` 时，`record_decision` **直接抛出异常** | `audit/log.py` |
...
| 要求 AI 助手代为决策的请求，**在调用模型之前就会被拒绝**，且每次返回相同结果 | `ai/conversation.py` |
...
使用随机种子 `20260815`，包含 6,000 笔交易，数据截止日为 2026-08-31。
...
仓库默认使用的随机种子(`20260815`)在召回率和精确率上**都是五个里最差的**——96.47% 和 77.83%。
它单次运行的混淆矩阵：**1,257** 真阳性、**358** 假阳性、**46** 假阴性、**4,339** 真阴性。

基线比例：发布种子 **21.72%** 应处理，跨 5 个种子 **22.35% ± 0.51%**
——这是**刻意富集**过的，真实跨境通道只有百分之一的零头。
**这些数字不能与生产环境类比。**

### 每个组件到底贡献了什么

如果只报告组合后的结果，就无法判断模型究竟贡献了什么：
...
- 朴素的「把模型删掉」比较显示 **+24.56pp 召回率**。那个差距是混合权重的算术:
  关闭模型会使每笔交易的综合分最多下降 0.30，但分流阈值并未同步调整。
...
| --- | --- |
| 按规范处理 | **100%**（24 / 24） |
| 要求 AI 代为决策，系统拒绝 | **100%**（8 / 8，中英文均覆盖） |
| 正常征求处置建议却被误拒 | **0**——“Should I hold this?” 是咨询意见，不是命令 AI 执行 |
| 询问系统并不掌握的数据，明确说明无法回答 | **100%** |
| 已回答轮次的引用可解析率 | **100%**(41 条引用，0 条无法解析) |
| 完全没有引用的回答 | **0** |

其中最容易混淆的是“征求建议”和“要求代为执行”，两者看似相近，处理方式却应完全不同。
*“这个付款人的信用分是多少？”* 含有「付款人」三个字——关键词路由会用钱包的支付历史来回答它:
回答可能很流畅、也带引用，但仍然答非所问。
反过来，如果安全规则连“Should I hold this?” 这样的正常咨询也拒绝，
分析师很快就会放弃使用这套工具。

### AI 简报质量：默认模式使用确定性模拟服务
...
处理时长及相关人工处置均为**模拟结果**，模拟过程采用固定的 12% 错误率。
它们描述的是模拟参数，不是一支真实团队。
...
| **调查工作台** —— 左边是证据，右边是**有什么会推翻它**,并排而不是上下，因为被队列压着的复核人是自上而下读、而且会提前停。 | **反洗钱预警队列** —— 按调查优先级排序，并按照设定的团队复核能力划分。线以下的部分被明确称为积压，而不是「已排除」。 |
| ![反洗钱运营](docs/screenshots/01-aml-operations-zh.png) | ![案件详情](docs/screenshots/06-case-detail-zh.png) |
| **反洗钱运营** —— 1,638 次原始触发 → 957 条预警 → 693 个案件 → 242 个能打开。这条漏斗展示了预警如何一步步转化为可调查案件。 | **案件详情** —— 支付侧的证据与仅供参考的简报。 |
...
| **审计日志** —— 哈希链式，含 AI 与人工的分歧矩阵。 | **追问** —— 分析师的第二个问题，基于更完整的案件上下文回答，同时拒绝代替分析师作出处置。 |
| ![策略调参](docs/screenshots/10-policy-tuning-zh.png) | ![评测](docs/screenshots/09-evaluation-zh.png) |
| **策略调参** —— 把两个自动化阈值当作产品决策，同时展示“召回率与人工复核率”的权衡曲线。 | **评测** —— 每一个指标，前提展开写在指标上方而不是脚注里。 |
...
| 如果你关心 | 从这里开始 | 论点是什么 |
| --- | --- | --- |
| **AI 产品 / Agent 安全** | [AI_BOUNDARIES.md](docs/AI_BOUNDARIES.md)，再阅读上文的追问对话 | 一条由数据结构、审计日志和两道护栏强制的 AI 权限边界——不是靠提示词。100% 的委托企图被拒绝，0 次由 AI 提交的决策 |
| **风控 / 支付** | [PRODUCT_CASE_STUDY.md](docs/PRODUCT_CASE_STUDY.md)，再查看案件详情页 | 跨境失败大多不是欺诈，所以这是一个混合队列的分诊系统，不是一个反欺诈检测器 |
| **评测 / 度量** | [EVALUATION_REPORT.md](docs/EVALUATION_REPORT.md) §1b–1d | 跨种子方差、如实呈现的组件基线，以及逐条消融实验 |
| **数据 / 分析** | [DATA_DICTIONARY.md](docs/DATA_DICTIONARY.md)，再查看策略调参页 | 每个指标在 SQL 中只定义一次；阈值是带可见成本曲线的产品决策 |
| **合规 / 审计** | [ARCHITECTURE.md](docs/ARCHITECTURE.md)，再查看审计日志页 | 哈希链式仅追加记录、理由码，以及被测量而非假设的误拦回收 |
...
`http://localhost:8501/?lang=zh` 直接进中文，因此可以直接分享指定语言的页面链接。

如需接入真实模型而非默认模拟服务，请将 `.env.example` 复制为 `.env`，设置
`RISKOPS_LLM_PROVIDER=openai_compatible` 并填写 API 密钥。核心处理流程不依赖模型，
...
- **未连接任何外部业务系统。** 没有真实网络、银行、钱包、商户或客户。
