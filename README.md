...
*Twenty seconds of the investigation workbench: one case, the eight factors that put it
first in the queue — all of them shown, so an investigator who disagrees can see which one
did it — and then the evidence, **beside** what would argue against it rather than above it.
That placement is the design: a reviewer under queue pressure reads top-down and stops early.*

**[▶ Full demo — 2:02](docs/demo/demo-zh-narrated.mp4)** · Chinese interface, narration, and
...
The guardrails above were built and measured against the deterministic mock. That is a real
limitation, and running two live models through the same pipeline is what exposed it.

`scripts/run_provider_comparison.py` regenerates briefs for a fixed sample of 24 resolved cases,
once per composer, with identical packets, prompt and guardrails.

| | mock | `deepseek-v4-pro` | + citation keys | `glm-5-2` | + citation keys |
| --- | ---: | ---: | ---: | ---: | ---: |
...
Both live models started at roughly 55% citation resolution, with over a hundred findings dropped
each. The cause was not fabricated facts. It was vocabulary: the system prompt showed the *shape*
of a citation by placeholder (`signal.RULE_ID`) but never listed the keys a packet actually
offers, so both models invented a JSON-path style of their own (`signals[0].rule_id`). The output
gate refused all of it — correctly, and silently, exactly as designed. Listing the allowed keys in
the packet moves resolution to ~98.5% and abstention to zero, with no change to the guardrail.

**The mock could not have found this.** It builds its citations from the same function that builds
the allowlist, so it cannot get the vocabulary wrong by construction. Its 100% was never evidence
that the gate works against a real model; it was evidence that the mock cannot violate it. That is
the honest limit of testing a guardrail with a stand-in.

One more thing the run confirmed: reasoning models bill their chain of thought against
`max_tokens`, so the 1,200-token product default truncates their JSON and drives unusable output
to 100%. The guardrail treats a truncated brief as no brief at all rather than as a partial
recommendation, which is the behaviour you want and a configuration default you have to fix.

Do not read the ground-truth row of that report as model skill — the mock's recommendation and the
generator's `expected_action` come from the same domain reasoning, so its agreement measures
internal consistency, not judgement. Full report and caveats:
...
python scripts/run_provider_comparison.py  # mock vs live models, needs a key
