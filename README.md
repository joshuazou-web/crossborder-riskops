...
riskops eval` on **synthetic** data, seed `20260815`, 6,000 transactions, cut-off 2026-08-31.
Full report with per-scenario and per-rule breakdowns: [EVALUATION_REPORT.md](docs/EVALUATION_REPORT.md).

### Risk detection

| Metric | Result | What it means |
| --- | --- | --- |
| **Recall** | **96.47%** | Of the 1,303 transactions labelled actionable, 1,257 were routed to a person or held. |
| **Precision** | **77.83%** | Of the 1,615 routed, 1,257 really were actionable. |
| **False-positive rate** | **7.62%** | 358 benign transactions cost a review. This is the price of the recall above. |
| **Manual review rate** | **26.92%** | The share of all traffic a person has to look at. |
| **Auto-release leakage** | **1.05%** | 46 actionable transactions were auto-released. These are the misses that matter. |

Confusion matrix: **1,257** true positives, **358** false positives, **46** false negatives,
**4,339** true negatives.

Base rate in this population: **21.72%** actionable — deliberately enriched, against a small
fraction of a percent in a real corridor. **These numbers are not comparable to production.**
